...
template <typename T, uint8_t N, T default_value> class FastRunningMedian {
  ...
  public:
    ...
    T getAverage(uint8_t n_medians) {
      if (n_medians > _window_size) {
        n_medians = _window_size;     // when filling the array for first time
      }

      T sum = 0;
      for (uint8_t i = _median_ptr - (n_medians / 2); i < (_median_ptr + (n_medians / 2)); i++) {
        sum += _sortbuffer[i];
      }
      return sum / n_medians;
    }
